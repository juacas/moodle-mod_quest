        ...
        if($calification_user = $DB->get_record("quest_calification_users",array("userid"=> $USER->id, "questid"=> $quest->id)))
        {
          if($quest->allowteams == 1)
          {
           if(empty($calification_user->teamid))
           {
            if(empty($_POST['team'])
/********
JPC: 20-11-2008: Patch: prevent creation of teams without name
*********/
 			|| trim($_POST['team'])=='')
            {
            	echo $OUTPUT->header();
            ...
            elseif(!empty($_POST['team']) && trim($_POST['team'])!='')
            {
   // team assignation or creation
            if($team = $DB->get_record("quest_teams", array("name"=>$_POST['team'], "questid"=>$quest->id, "currentgroup"=> $currentgroup)))
            {
...
JPC: 20-11-2008: Patch: prevent creation of teams without name
*********/
	if ($team->name == '') $team->name='Team_user('.$calification_user->id.")";
...
*************************/
    ...
    add_to_log($course->id, "quest", "view", "view.php?id=$cm->id", "$quest->id", "$cm->id");
	echo $OUTPUT->header();
    ...
    /***********************************************************************************************************************************************************************************************************************************************************************************/
    elseif ($action == 'teachersview' || $action == 'studentsview')
    {
...
//         }
        ...
        /// Print admin links
        echo '<td align="right"><a href="getLogs.php?qid='.$cm->id.'">Log</a>';
        echo '<td align="right"><a href="report.php?cmid='.$cm->id.'">Briefting.</a>';
        echo '</td></tr>';
        echo "</table>";
        quest_print_quest_heading($quest);
        ...
        if (has_capability('mod/quest:manage',$context) and $quest->nelements) {
        	$edit_icon= $OUTPUT->pix_icon('t/edit', get_string('amendassessmentelements','quest'));
        	$text .="&nbsp;<a href=\"assessments.php?cmid=$cm->id&newform=0&cambio=0&amp;viewgeneral=1&amp;action=editelements\">".
              $edit_icon.'</a>';
        }
		$text .= "</b></center>";
        echo($text);
        ...
        echo "</td></tr>";
        echo "<tr><td width=\"70%\" valign=\"top\">";
        //print_simple_box(format_text($quest->description), 'left','100%');
        echo $OUTPUT->box(format_text($quest->description));
        echo "</td><td width=\"30%\" valign=\"top\">";
		...
		elseif($quest->dateend > $timenow)
		{
          echo( "<a href=\"submissions.php?action=submitexample&amp;cmid=$cm->id\">".
        ...
        echo $OUTPUT->help_icon('submitexampleassignment','quest');
		echo '<br/>';
?>
 ...
 */
        ...
        foreach ($submissions as $submission)
		{
			...
			$user=$DB->get_record('user', array('id'=> 1)); // Guest user
			else
			...
			if (
				...
				&& !has_capability('mod/quest:manage', $context)
       ...
       if ($REPEAT_ACTIONS_BELOW)
        {
        	if($quest->dateend > $timenow){
        		echo( "<center><b><a href=\"view.php?action=submitexample&amp;id=$cm->id\">".
