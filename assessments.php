...
*******************************************************/
    ...
    $action=required_param('action',PARAM_ALPHA);
  	$sid=optional_param('sid','',PARAM_INT);     // Quest Submission ID
  	$newform=optional_param('newform','',PARAM_INT);	//Flag: if you want new form for one submission...newform=1. If form is general...newform=0
  	$num_elems_when_change=optional_param('num_elems_when_change','',PARAM_INT); //New number of elements when you add or remove elements
  	$change_form=optional_param('change_form','',PARAM_INT); //Flag: if you change the number of elements in forms, change_form=1, else 0
    ...
    if ($id) {
        ...
        if (! $quest = $DB->get_record("quest", array("id"=> $qid))) {
            error("Quest id is incorrect");
        }
...
*/
    ...
    if ($num_elems_when_change!= ''){
    	$url->param('$num_elems_when_change', $num_elems_when_change);
    }
    if ($change_form!= ''){
    	$url->param('$change_form', $change_form);
    }
    if ($viewgeneral!== -1){
    ...
    elseif ($action == 'editelements') {
       	...
       	if ($newform==0)
        {
       	...
       	if ($elementsraw = $DB->get_records("quest_elements", array("submissionsid"=> $sid_target), "elementno ASC" )) {
        ...
        //if ($DB->count_records("quest_submissions", "numelements", "id", $sid)==0) //evp esta linea no s� c�mo cambiarla , estar�a mal, propongo algo a ver si es lo que se pretend�a
        //evp creo que lo que pretenden es que si no hay nada creado de elementos para ese desaf�o, ponemos n�mero de elementos es de por defecto del quest. HAY QUE COMPROBARLO
        // EVP PROBAR ESTO MUY BIEN!!!
        if($DB->count_records('quest_elements',array('submissionsid'=>$sid))==0)
        {
        	$num=$quest->nelements;
        }
        if (($newform==1)&&($change_form==1))
        {
        	$num=$num_elems_when_change;
        }
        if (($DB->get_field ("quest_submissions", "numelements", array("id"=> $sid))!=0)&&($change_form==0)&&($newform==1))
        {
        	$num = $DB->get_field ("quest_submissions", "numelements", array("id"=> $sid));
        }
        //if (($DB->get_field ("quest_submissions", "numelements", "id", $sid)!=0)&&($change_form==1)&&($newform==0))
        if (($change_form==1)&&($newform==0))
        {
        	$num=$num_elems_when_change;
        }
        if (($newform==0)&&($change_form==0))
        {
        	$num=$quest->nelements;
        }
...
// TODO: replace  with quest_print_assessment from locallib.php
        ...
        switch ($quest->gradingstrategy) {
            ...
            case 4: // rubric
				...
				if ($rubricsraw = $DB->get_records_select("quest_rubrics", "questid = ? AND submissionsid = ?", array($quest->id,$var), "elementno ASC")) {
    ...
    elseif ($action == 'insertelements') {
            ...
            case 1: // accumulative grading
                ...
                foreach ($form->description as $key => $description) {
                    if ($description) {
						...
						else if (($newform==0)||(($DB->count_records("quest_elements", array("questid"=> $quest->id, "submissionsid"=> 0))==0)))
						{
							$element->submissionsid = 0;
						}
                        $element->elementno = $key;
        ...
        {
        	echo $OUTPUT->redirect_message("submissions.php?cmid=$cm->id&sid=$sid&action=showsubmission", get_string("savedok","quest"),1,false);
        }
    }
...
//            		}
         ...
         { // si no es profesor la fase siempre será phase=0. La nota queda pendiente
     ...
     { // Este QUEST no requiere validación
...
// la actual es la nueva correcta y hay que recalificar el resto.
                 if($submission->nanswerscorrect > 0)
                 	...
                 	elseif($submission->nanswerscorrect == 0)
                 	{
                 		$recalification = true;
                 		$submission->dateanswercorrect=$answer->date;
                 	}
...
//update pointsanswercorrect
	if($query  = $DB->get_record_select("quest_answers","submissionid=? and grade>=50", array($submission->id),"date,pointsmax",IGNORE_MULTIPLE))
	{
...
// state 0 no realizada 1 por autor 2 por profesor.
		...
		quest_update_assessment($assessment);// weird bug with number precission and decimal point in Moodle 2.5+
	...
	$returnto="view.php?id=$cm->id";
        ...
        if ($quest->gradingstrategy)
        {
        	echo $OUTPUT->redirect_message($returnto, 
        								get_string("thegradeis", "quest").": ".number_format($grade , 4)." (".get_string("maximumgrade")." ".number_format($points,4).")",
        								10,
        								false);
        }
        else {
