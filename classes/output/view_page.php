...
class view_page implements renderable, templatable {
    ...
    protected function render_card_sparkline($c, int $tinitial, int $timenow): string {
        ...
        $ymax = max($pmax * 1.1, $pinit * 1.15, 10);
        $ymin = 0;
        ...
        $svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' . $width . ' ' . $height . '" class="w-100 quest-card-sparkline" style="height: 42px; border-radius: 4px; background: rgba(0,0,0,0.02);">';

        // Phase background zones:
        $svg .= '<rect x="' . $startX . '" y="' . $padTop . '" width="' . max(0, $statX - $startX) . '" height="' . $plotHeight . '" fill="rgba(13, 110, 253, 0.08)" />';
        $svg .= '<rect x="' . $statX . '" y="' . $padTop . '" width="' . max(0, $inflX - $statX) . '" height="' . $plotHeight . '" fill="rgba(245, 158, 11, 0.09)" />';
        if ($hasCorrect) {
            $svg .= '<rect x="' . $inflX . '" y="' . $padTop . '" width="' . max(0, $endX - $inflX) . '" height="' . $plotHeight . '" fill="rgba(139, 92, 246, 0.09)" />';
        }

        // Tramos curves:
        // Tramo 1: Estacionario (blue)
        $svg .= '<line x1="' . $startX . '" y1="' . $statY . '" x2="' . $statX . '" y2="' . $statY . '" stroke="#0d6efd" stroke-width="2.5" stroke-linecap="round" />';

        // Tramo 2: Inflacionario (amber)
        $svg .= '<line x1="' . $statX . '" y1="' . $statY . '" x2="' . $inflX . '" y2="' . $inflEndY . '" stroke="#f59e0b" stroke-width="2.5" stroke-linecap="round" />';

        // Tramo 3: Deflacionario (purple)
        if ($hasCorrect) {
            $deflEndY = $gety($pmin);
            $svg .= '<line x1="' . $inflX . '" y1="' . $inflEndY . '" x2="' . $endX . '" y2="' . $deflEndY . '" stroke="#8b5cf6" stroke-width="2.5" stroke-linecap="round" />';
        } else {
            $projStartY = $gety($pmax);
            $projEndY = $gety($pmin);
            $svg .= '<line x1="' . $inflX . '" y1="' . $projStartY . '" x2="' . $endX . '" y2="' . $projEndY . '" stroke="#8b5cf6" stroke-width="1.5" stroke-dasharray="3,2" opacity="0.5" />';
        }
        ...
        if ($timenow >= $datestart && $timenow <= $dateend) {
            ...
            $svg .= '<line x1="' . $nowX . '" y1="' . $padTop . '" x2="' . $nowX . '" y2="' . ($padTop + $plotHeight) . '" stroke="#198754" stroke-width="1.5" stroke-dasharray="2,2" />';
            $svg .= '<circle cx="' . $nowX . '" cy="' . $nowY . '" r="3" fill="#198754" stroke="#fff" stroke-width="1" />';
        }
