...
$questid=$quest->id;
$userid="1294";
//$query = $DB->get_record_select("quest_assessments_autors","questid=$questid and userid in ($userid)","sum(points) as points");
//	print_object($query);
...
//evp revisar que funciona bien, esta sentencia $sql no se usa en ning�n lado es probable que se olvidasen de borrarla
$sql="select sum(ans.grade*ans.pointsmax/100) as points from {quest_answers} as ans, {quest_assessments} as assess WHERE " .
	"ans.questid=$questid AND " .
	"ans.userid in ($userid)AND " .
