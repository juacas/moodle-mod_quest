...
*******************/
   ...
   if(isset($form->operation))
   {
    if($form->operation == get_string("approve","quest"))
    {
         ...
         $newsubmission->title  = $form->title;
          ...
          $newsubmission->datestart = make_timestamp($form->submissionstartyear,
            $form->submissionstartmonth, $form->submissionstartday, $form->submissionstarthour,
            $form->submissionstartminute);

          $newsubmission->dateend = make_timestamp($form->submissionendyear,
            $form->submissionendmonth, $form->submissionendday, $form->submissionendhour,
            $form->submissionendminute);
    ...
    else if($form->operation == get_string("save","quest"))
    {
        ...
        $DB->delete_records('event', array('modulename'=> 'quest', 'instance'=>$quest->id, 'description'=> $submission->description));
   ...
   if($form->save == "submitassignment")
   {
    ...
    if($calification_user = $DB->get_record("quest_calification_users", array("userid"=>$USER->id, "questid"=> $quest->id))){
        $calification_user->nsubmissions++;
        $DB->set_field("quest_calification_users","nsubmissions", $calification_user->nsubmissions, array("id"=> $calification_user->id));
    ...
    if($quest->allowteams)
    {
     if($calification_team = $DB->get_record("quest_calification_teams", array("teamid"=> $calification_user->teamid, "questid"=> $quest->id))){
         $calification_team->nsubmissions++;
         $DB->set_field("quest_calification_teams","nsubmissions", $calification_team->nsubmissions, array("id"=> $calification_team->id));
    ...
    if($ismanager || has_capability('quest:approvechallenge', $context))
    {
     if($newsubmission->datestart <= time()) // TODO: Check. This avoids lo log a challenge if started retrospectively
     {
