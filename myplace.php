...
***********************************/
   ...
   $timenow = time();
    ...
    $url =  new moodle_url('/mod/quest/myplace.php',array('id'=>$id, 'action'=>$action, 'sort'=>$sort,'dir'=>$dir,'sortanswer'=>$sortanswer, 'diranswer'=>$diranswer));
    ...
    add_to_log($course->id, "quest", "view_myplace", "myplace.php?id=$cm->id", "$quest->id","$cm->id");
    ...
    if ($groupmode and $ismanager) {
        if ($groups = $DB->get_records_menu("groups", array("courseid"=> $course->id), "name ASC", "id,name")) {
                ...
                //print_group_menu($groups, $groupmode, $currentgroup, "myplace.php?id=$cm->id");
                groups_print_activity_menu($cm, $CFG->wwwroot."mod/quest/myplace.php?cmid=$cm->id", $return=false, $hideallparticipants=false);//evp revise this 
    ...
    $title = get_string('myplace','quest');
    echo $OUTPUT->heading_with_help($title,"myplace","quest");

    $text = '';
    $text = "<center><b>";
    if($quest->dateend > $timenow){
    $text .= "<a href=\"submissions.php?action=submitexample&amp;cmid=$cm->id\">".
    ...
    if($ismanager and $quest->nelements){
     echo "<center>(<a href=\"assessments_autors.php?id=$cm->id&amp;action=editelements\">"
                .get_string('editelementsautor','quest')."</a> / <a href=\"assessments.php?cmid=$cm->id&newform=0&amp;action=editelements\">".get_string('editelementsanswer','quest')."</a>)</center>";
    }
...
<?PHP
        ...
        $indice = 0;
            ...
            if ($submissions = quest_get_user_submissions($quest, $USER)) {
                foreach ($submissions as $submission) {
                  ...
                  if( $submission->userid == $USER->id){
                   ...
                   elseif(($submission->nanswers == 0)and($timenow < $submission->dateend)and($submission->state < 2)){
		    ...
		    $columns[]='grade';
		    $table->align[]='center';
		if ($submission->evaluated!=0 && $assessment = $DB->get_record("quest_assessments_autors",array("questid"=> $quest->id,
		             "submissionid"=>$submission->id)))
			{
				if($submission->pointsanswercorrect>0){
		    		$data[]= number_format($assessment->points*100/$submission->pointsanswercorrect,1).'% ('.number_format($assessment->points,4).')';
				}else{
        ...
        $grafic = "<img src=\"".$CFG->wwwroot."/pix/t/clear.png\" />";
        echo "<center>";
            ...
            if ($answers = $DB->get_records_select("quest_answers", "questid=? AND userid=?", array($quest->id,$USER->id))) {
                foreach ($answers as $answer) {
                   ...
                   if($answer->userid == $USER->id){
                    ...
                    elseif(($answer->userid == $USER->id)&&($submission->dateend > $timenow)&&($answer->phase == 0)){
                     $data[] = quest_print_answer_title($quest, $answer,$submission).
                         " <a href=\"answer.php?action=modif&amp;id=$cm->id&amp;aid=$answer->id&amp;sid=$submission->id\">".
                         "<img src=\"".$CFG->wwwroot."/pix/t/edit.svg\" ".
                         'height="11" width="11" border="0" alt="'.get_string('modif', 'quest').'" /></a>'.
    ...
    if($quest->allowteams){
          $table->head = array ("",get_string('firstname','quest').'/'.get_string('lastname','quest'), get_string('nanswers','quest'), get_string('nanswersassessment','quest'), get_string('nsubmissions','quest'), get_string('nsubmissionsassessment','quest'), get_string('pointssubmission','quest'), get_string('pointsanswers','quest'), get_string('pointsteam','quest'), get_string('points','quest'));
    }
    else{

          $table->head = array ("",get_string('firstname','quest').'/'.get_string('lastname','quest'), get_string('nanswers','quest'), get_string('nanswersassessment','quest'), get_string('nsubmissions','quest'), get_string('nsubmissionsassessment','quest'), get_string('pointssubmission','quest'), get_string('pointsanswers','quest'), get_string('points','quest'));
    }
   ...
   if((!$ismanager)&&($quest->allowteams)){
    ...
    /// Now prepare table with student assessments and submissions
        ...
        if ($clasification_user = $DB->get_record("quest_calification_users", array("userid"=> $USER->id, "questid"=> $quest->id))) {
           if($calification_team = $DB->get_record("quest_calification_teams", array("teamid"=>  $clasification_user->teamid, "questid"=> $quest->id)))
			{
        ...
        $table->head = array (get_string('team','quest'), get_string('nanswers','quest'), get_string('nanswersassessment','quest'), get_string('nsubmissions','quest'), get_string('nsubmissionsassessment','quest'), get_string('pointssubmission','quest'), get_string('pointsanswers','quest'), get_string('points','quest'));
        ...
        if ($REPEAT_ACTIONS_BELOW)
        {
          ...
          get_string('viewclasificationteams', 'quest')."</a>";
        	...
        	if(isteacheredit($course->id) and $quest->nelements){
        		echo "<center>(<a href=\"assessments_autors.php?id=$cm->id&amp;action=editelements\">"
        		.get_string('editelementsautor','quest')."</a> / <a href=\"assessments.php?cmid=$cm->id&amp;action=editelements\">".get_string('editelementsanswer','quest')."</a>)</center>";
        	}
        }
